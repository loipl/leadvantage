...
DATE(d.request_time) AS `date`,  d1.value AS `gender`, d2.value AS `first_name`,
...
LEFT JOIN profiles_data AS d1 ON a.id = d1.profile_id AND d1.field_type_id = 18
LEFT JOIN profiles_data AS d2 ON a.id = d2.profile_id AND d2.field_type_id = 7
WHERE
a.email IS NOT NULL 
...
header("Content-Disposition: attachment; filename=shannon-dating.csv");
