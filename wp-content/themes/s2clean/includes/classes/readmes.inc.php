...
/**/
if (!class_exists ("c_ws_theme__readmes"))
	{
		class c_ws_theme__readmes
			{
				/*
				...
				public static function parse_readme ($specific_path = FALSE, $specific_section = FALSE, $_blank_targets = TRUE, $process_wp_syntax = FALSE)
					{
						if (!($path = $specific_path)) /* Was a specific path passed in? */
							{
								$path = TEMPLATEPATH . "/readme.txt";
								$dev_path = TEMPLATEPATH . "/readme-dev.txt";
								$path = (file_exists ($dev_path)) ? $dev_path : $path;
							}
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_theme__before_parse_readme", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (file_exists ($path)) /* Gives hooks a chance. */
							{
								$o_pcre = @ini_get ("pcre.backtrack_limit");
								@ini_set ("pcre.backtrack_limit", 10000000);
								/**/
								if (!function_exists ("NC_Markdown"))
									include_once TEMPLATEPATH . "/includes/_xtnls/markdown/nc-markdown.inc.php";
								/**/
								$rm = file_get_contents ($path); /* Get readme.txt file contents. */
								$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($rm, "UTF-8", $GLOBALS["WS_THEME__"]["c"]["mb_detection_order"]) : $rm;
								...
								if ($specific_section) /* If we are ONLY parsing a specific section. This is a very useful way of pulling details out. */
									{
										...
										$readme .= '</div>' . "\n";
										/**/
										return apply_filters ("ws_theme__parse_readme", $readme, get_defined_vars ());
									}
								else /* Otherwise, we're going for the entire readme file. Here we have lots of work to do. */
									{
										...
										$rm = stripslashes (preg_replace ($r3, "'$1$2$3$4$5'.c_ws_theme__readmes::_parse_readme_specs('$6').'$7'", $rm, 1));
				...
				*/
				public static function _parse_readme_specs ($str = FALSE)
					{
						do_action ("_ws_theme__before_parse_readme_specs", get_defined_vars ());
						/**/
						...
						/**/
						return apply_filters ("_ws_theme__parse_readme_specs", $str, get_defined_vars ());
					}
				/*
				...
				public static function parse_readme_value ($key = FALSE, $specific_path = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_theme__before_parse_readme_value", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($readme[$path] || file_exists ($path))
							{
								if (!$readme[$path]) /* If not already opened, we need open it up now. */
									{
										...
										$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($readme[$path], "UTF-8", $GLOBALS["WS_THEME__"]["c"]["mb_detection_order"]) : $readme[$path];
								...
								/**/
								return strlen ($m[5] = trim ($m[5])) ? apply_filters ("ws_theme__parse_readme_value", $m[5], get_defined_vars ()) : false;
							}
						else /* Nope. */
							return false;
					}
