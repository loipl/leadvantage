...
function optpress_video( $atts, $content = null ) {
	...
	$out .= '<param name="stretching" value="exactfit">';
	$out .= '<param name="controlbar" value="over">';
	$out .= '<param name="flashvars" value="file='.$video_url.'&stretching=fill&image='.$video_image.'&controlbar=over">';
	$out .= '<embed id="'.$video_id.'" name="'.$video_id.'" src="'.get_bloginfo('template_directory').'/js/player.swf" width="'.$width.'" height="'.$height.'" allowscriptaccess="always" allowfullscreen="true" flashvars="file='.$video_url.'&stretching=exactfit&image='.$video_image.'&controlbar=over&screencolor=FFFFFF&backcolor=1b1b1b&frontcolor=FFFFFF&lightcolor=1173ba"/>';
	$out .= '</object>';
	$out .= do_shortcode($content);
