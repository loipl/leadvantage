						...
						if (file_exists ($path)) /* Give hooks a chance. */
							{
								...
								$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($rm, "UTF-8", $GLOBALS["WS_WIDGET__"]["ad_squares"]["c"]["mb_detection_order"]) : $rm;
								...
								if (!$readme[$path]) /* If not already opened, we need open it up now. */
									{
										...
										$mb = function_exists ("mb_convert_encoding") ? @mb_convert_encoding ($readme[$path], "UTF-8", $GLOBALS["WS_WIDGET__"]["ad_squares"]["c"]["mb_detection_order"]) : $readme[$path];
