...
$userIDs = listEligibleUserIDs($lwDb, array('FIN','EDU','INS'));
if (!$userIDs) {
...
WHERE
d.request_time < DATE_ADD(NOW(), INTERVAL -7 DAY) 
AND d.request_time > DATE_ADD(NOW(), INTERVAL -$days DAY) 
...
header("Content-Disposition: attachment; filename=marc.csv");
