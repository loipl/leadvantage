...
$userIDs = listEligibleUserIDs($lwDb, array('FIN'));
if (!$userIDs) {
...
WHERE
d.request_time < DATE_ADD(NOW(), INTERVAL -30 DAY) 
AND d.request_time > DATE_ADD(NOW(), INTERVAL -120 DAY) 
AND a.email IS NOT NULL
AND (c1.value = 'Germany' OR c1.value = 'Canada' OR c1.value = 'India' OR c1.value = 'China' OR c1.value = 'Brazil' OR c1.value = 'Mexico')
AND (a.email LIKE '%@yahoo.%' OR a.email LIKE '%@ymail.%' OR a.email LIKE '%@aol.%' OR a.email LIKE '%@hotmail.%' OR a.email LIKE '%@outlook.%' OR a.email LIKE '%@sbcglobal.net')
...
header("Content-Disposition: attachment; filename=andre-other.csv");
